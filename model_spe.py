...
class GPT(nn.Module):
    ...
    def __init__(self, config):
        ...
        # 2. Create the sinusoidal positional encoding matrix
        spe = torch.zeros(config.block_size, config.n_embd)
        ...
        spe[:, 0::2] = torch.sin(position * div_term)
        spe[:, 1::2] = torch.cos(position * div_term)

        # 3. Register 'spe' as a buffer. 
        # 'spe' will be a fixed tensor (shape [block_size, n_embd])
        # and will be moved to the correct device (e.g., GPU)
        # automatically when you call .to(device) on the model.
    ...
    def forward(self, idx, targets=None):
        ...
        pos_emb = self.spe[:t, :] # position embeddings of shape (t, n_embd)
