...
from model_sparseDep import GPTConfig, GPT
...
ckpt_path = 'ckpt_softmax_H6_HDim32'
# wandb logging
...
def get_batch(split):
    ...
    perms = np.memmap(os.path.join('data', f'perms_blkSize{block_size}.bin'), dtype=np.uint16, mode='r')

    ix = torch.randint(len(data) - block_size, (batch_size,))
    x = torch.stack([torch.from_numpy((data[i:i+block_size]).astype(np.int64)) for i in ix])
    y = torch.stack([torch.from_numpy((data[i+1:i+1+block_size]).astype(np.int64)) for i in ix])

    def get_masks(batch_size, block_size, sparsity, device):
        num_false_cols = int(round(block_size * sparsity))

        pix = torch.randint(len(perms)//block_size, (batch_size,)) * block_size
        false_indices = torch.stack([torch.from_numpy((perms[i: i+num_false_cols]).astype(np.int64)) for i in pix])
        false_indices = false_indices.to(device)

        # Start with all True
        mask = torch.ones(batch_size, block_size, block_size, dtype=torch.bool, device=device)
        # For each batch b, zero specific columns
        mask.scatter_(2, false_indices.unsqueeze(1).expand(-1, block_size, -1), False)

        return mask
    m = get_masks(batch_size, block_size, sparsity, device)
    ...
    return x, y, m.view(batch_size, 1, block_size, block_size)
...
# training loop
X, Y, M = get_batch('train') # fetch the very first batch
t0 = time.time()
